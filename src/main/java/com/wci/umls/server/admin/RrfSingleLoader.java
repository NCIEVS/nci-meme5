...
/**
 * Admin tool which loads a single RRF terminology into a database.
 *
 * <p>Usage:
 * <pre>
 *   ./gradlew adminLoadRrfSingle -Pterminology=SNOMEDCT_US -Pversion=latest \
 *       -Pprefix=MR -Pinput.dir=/data/rrf -Pserver=false -Pmode=create
 * </pre>
 */
public class RrfSingleLoader extends AbstractLoader {

  /** Logger. */
  private static final Logger LOG =
      Logger.getLogger(RrfSingleLoader.class.getName());
  ...
  public void run() throws Exception {
    ...
    LOG.info("RRF Single Terminology Loader");
    ...
    if (!serverRunning) {
      ...
      contentService.loadTerminologyRrf(terminology, version,
          RrfLoaderAlgorithm.Style.SINGLE.toString(), prefix, inputDir,
          authToken);
    } else {

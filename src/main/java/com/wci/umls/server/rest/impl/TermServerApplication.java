...
/** The application (for jersey). Also serves the role of the initialization listener. */
@ApplicationPath("/")
  ...
  public TermServerApplication() throws Exception {
    ...
    try {
      if (!ConfigUtility.getConfigProperties().containsKey("deploy.mode")
          || !ConfigUtility.getConfigProperties().getProperty("deploy.mode").contains("PROD")) {
     ...
     *
     * <p>// Instantiate bean config BeanConfig beanConfig = new BeanConfig();
     * beanConfig.setTitle("Term Server API"); beanConfig.setDescription("RESTful calls for
     * terminology server"); beanConfig.setVersion(API_VERSION); final URL url = new
     * URL(ConfigUtility.getConfigProperties().getProperty("base.url")); final String host =
     * url.getHost() + ":" + url.getPort();
     *
     * <p>if (new ConfigureServiceRestImpl().isConfigured()) { beanConfig.setHost(host);
     * beanConfig.setBasePath(url.getPath()); beanConfig.setSchemes(new String[] { url.getProtocol()
     * }); beanConfig.setResourcePackage("com.wci.umls.server.rest.impl"); beanConfig.setScan(true);
     * beanConfig.setPrettyPrint(true); }
     *
     * <p>// this makes Swagger honor JAXB annotations Json.mapper().registerModule(new

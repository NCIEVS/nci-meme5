  ...
  public void compute() throws Exception {
    ...
    try {
      ...
      for (final String line : lines) {
        ...
        // There's an edge-case where MTH/PN atoms may have been turned off by
        // editors, so need to load unpublishable atoms only for MTH insertions.
        final Atom oldAtom = !getProcess().getTerminology().equals("MTH")
            ? (Atom) getComponent("AUI", newAtomAui, null, null)
            : (Atom) getComponent("AUI", newAtomAui, null, null, true);

        // Check if existing atom has already been processes on a previous run.
        // In this case, skip to the next line
        if (oldAtom != null
            && oldAtom.getLastModifiedBy().equals(getLastModifiedBy())) {
      ...
      if (getProcess().getTerminology().equals("MTH")) {
        ...
        for (final String line2 : lines2) {
          ...
          // Since we've moved SNOMED into a separate insertion, skip
          // SNOMEDCT_US lines.
          ...
          if (stringClassId != null) {
            ...
            if (atomAui != null) {
              ...
              final Atom atom =
                  (Atom) getComponent("AUI", atomAui, null, null, true);

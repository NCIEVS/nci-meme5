...
public class AddRemoveIntegrityCheckAlgorithm
  ...
  public void compute() throws Exception {
    ...
    logInfo("  addRemove = " + addRemove);
    logInfo("  checkName = " + checkName);
    logInfo("  value1 = " + value1);
    logInfo("  value2 = " + value2);
    commitClearBegin();
    ...
    try {
      ...
      final List<TypeKeyValue> validationData =
          getProject().getValidationData();

      if (addRemove.equals("Remove")) {
        final List<TypeKeyValue> list = findTypeKeyValuesForQuery("type:"
            + checkName + " AND key:\""
            + value1 + "\"" + (!ConfigUtility.isEmpty(value2)
                ? " AND NOT value:[* TO *]" : " AND value:\"" + value2 + "\""),
            null).getObjects();
        if (list.size() > 1) {
          throw new Exception("Unexpected number of type key values found: "
              + checkName + ", " + value1 + ", " + value2);
        }
        if (list.size() == 1) {
          final TypeKeyValue validationCheckData = list.get(0);
          if (validationData.contains(validationCheckData)) {
            // Remove from the project and update
            getProject().getValidationData().remove(validationCheckData);
            updateProject(getProject());

            // Remove from the DB
            logInfo("  remove = " + validationCheckData);
            removeTypeKeyValue(validationCheckData.getId());
          } else {

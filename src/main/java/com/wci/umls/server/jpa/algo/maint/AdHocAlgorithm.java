...
import com.wci.umls.server.jpa.model.content.ComponentHistoryJpa;
...
public class AdHocAlgorithm extends AbstractInsertMaintReleaseAlgorithm {
  ...
  public void compute() throws Exception {
    ...
    logInfo("  integerParameter = " + integerParameter);
    logInfo("  stringParameter = " + stringParameter);
    ...
    } else if (actionName.equals("Set Component Info Relationships To Publishable")) {
      setComponentInfoRelationshipsToPublishable();
    } else if (actionName.equals("Set Stamped Worklists To Ready For Publication")) {
  ...
  private void undoStampings() throws Exception {
    ...
    final MolecularActionList molecularActions = findMolecularActions(null,
        getProject().getTerminology(), getProject().getVersion(), "activityId:" + activityId, pfs);

    for (final MolecularAction molecularAction : molecularActions.getObjects()) {

      if (molecularAction.isUndoneFlag()) {
        logInfo("Already undone: molecularAction=" + molecularAction.getId() + ", for concept="
            + molecularAction.getComponentId());
        successful++;
      }
        ...
        try {
          ...
          logInfo("Could not undo molecularAction=" + molecularAction.getId() + ", for concept="
              + molecularAction.getComponentId());
          unsuccessful++;
        } finally {
          undoAction.close();
    ...
    logInfo("[UndoStampings] " + successful + " stampings successfully undone.");
  ...
  private void fixOrphanDefinitions() throws Exception {
    ...
    for (Map.Entry<Long, Long> entry : definitionIdAtomIdMap.entrySet()) {
      ...
      if (atom.getDefinitions().contains(definition)) {
        logWarn("atom=" + atomId + " already contains definition=" + definitionId);
        continue;
      }
    ...
    logInfo(
        "[FixOrphanDefinitions] " + successful + " orphan definitions successfully reattached.");
  ...
  private void removeBadRelationships() throws Exception {
    ...
    logInfo("[RemoveBadRelationships] " + relIds.size() + " ConceptRelationship ids loaded");

    for (Long id : relIds) {
      final ConceptRelationship rel =
          (ConceptRelationshipJpa) getRelationship(id, ConceptRelationshipJpa.class);
      ...
      // its inverse
        ...
        try {
          inverseRel =
              (ConceptRelationshipJpa) getInverseRelationship(getProject().getTerminology(),
                  getProject().getVersion(), rel);
        } catch (Exception e) {
          logInfo(
              "[RemoveBadRelationships] Could not find inverse relationship for: " + rel.getId());
        }

        if (inverseRel != null) {
          logInfo("[RemoveBadRelationships] Removing overlapping inverse relationship: "
              + inverseRel.getId());
    ...
    logInfo("[RemoveBadRelationships] " + removals + " bad relationships successfully removed.");
  ...
  private void fixScrAtomIds() throws Exception {
    ...
    logInfo("[FixSrcAtomIds] " + successful + " SRC atom Id successfully updated.");
  }
  ...
  private void redoMolecularActions() throws Exception {
    ...
    try {
      ...
      List<Long> molecularActionIds = new ArrayList<>(Arrays.asList(3204914L, 3204915L, 3204916L,
          3204917L, 3204918L, 3204919L, 3204920L, 3204921L, 3204922L, 3204923L, 3204924L, 3204925L,
          3204926L, 3204927L, 3204928L, 3204929L, 3204930L, 3204931L, 3204932L, 3204933L, 3204934L,
          3204935L, 3204936L, 3204937L, 3204938L, 3204939L, 3204940L, 3204941L, 3204942L, 3204943L,
          3204944L, 3204945L, 3204946L, 3204947L, 3204948L, 3204949L, 3204950L, 3205251L, 3205252L,
          3205253L, 3205254L, 3205255L, 3205256L, 3205257L, 3205258L, 3205259L, 3205260L, 3205261L,
          3205262L, 3205263L, 3205264L, 3205265L, 3205266L, 3205267L, 3205268L, 3205269L, 3205270L,
          3219303L, 3219304L, 3219305L, 3219306L, 3219307L, 3219308L, 3219309L, 3219310L, 3219311L,
          3219312L, 3219313L, 3219314L, 3219315L, 3219316L, 3219317L, 3219318L, 3219319L, 3219320L,
          3219321L, 3219322L, 3219323L, 3219324L, 3219325L, 3219326L, 3219327L, 3219328L, 3219329L,
          3219330L, 3219331L, 3219332L, 3219333L, 3219334L, 3219335L, 3219336L, 3219337L, 3219338L,
          3219339L, 3219340L, 3219341L, 3219342L, 3219343L, 3219344L, 3219345L, 3219346L, 3219347L,
          3219348L, 3219349L, 3219350L, 3219651L, 3219652L, 3219653L, 3219654L, 3219655L, 3219656L,
          3219657L, 3219658L, 3219659L, 3219660L, 3219661L, 3219662L, 3219663L, 3219664L, 3219665L,
          3219666L, 3219667L, 3219668L, 3219669L, 3219670L, 3219671L, 3219672L, 3219673L, 3219674L,
          3219675L, 3219676L, 3219677L, 3219678L, 3219679L, 3219680L, 3219681L, 3219682L, 3219683L,
          3219684L, 3219685L, 3219686L, 3219687L, 3219688L, 3219689L, 3219690L, 3219691L, 3219692L,
          3219693L, 3219694L, 3219695L, 3219696L, 3219697L, 3219698L, 3219699L, 3219700L, 3220001L,
          3220002L, 3220003L, 3220004L, 3220005L, 3220006L, 3220007L, 3220008L, 3220009L, 3220010L,
          3220011L, 3220012L, 3220013L, 3220014L, 3220015L, 3220016L, 3220017L, 3220018L, 3220019L,
          3220020L, 3220021L, 3220022L, 3220023L, 3220024L, 3220025L, 3220026L, 3220027L, 3220028L,
          3220029L, 3220030L, 3220031L, 3220032L, 3220033L, 3220034L, 3220035L, 3220036L, 3220037L,
          3220038L, 3220039L, 3220040L, 3220041L, 3220042L, 3220043L, 3220044L, 3220045L, 3220046L,
          3220047L, 3220048L, 3220049L, 3220050L, 3220301L, 3220302L, 3220303L, 3220304L, 3220305L,
          3220306L, 3220307L, 3220308L, 3220309L, 3220310L, 3220311L, 3220312L, 3220313L, 3220314L,
          3220315L, 3220316L));

      for (Long molecularActionId : molecularActionIds) {

        final MolecularAction molecularAction = getMolecularAction(molecularActionId);
        // Create and set up an undo action
        ...
        redoAction.performMolecularAction(redoAction, getLastModifiedBy(), false, false);
  ...
  private void fixBadRelationshipIdentities() throws Exception {
    ...
    try {
      ...
      logInfo("[FixBadRelationshipIdentities] Identifying " + "duplicate relationship identities");
      ...
      for (final RelationshipIdentity identity : relationshipIdentities) {
        ...
        List<RelationshipIdentity> duplicateRelationshipIdentities = new ArrayList<>();
        List<Long> duplicateRelationshipIds = new ArrayList<>();

        // get the identity's duplicate(s)
        query = getEntityManager().createQuery("select r.id from " + "RelationshipIdentityJpa r "
        ...
        List<RelationshipIdentity> inverseRelationshipIdentities = new ArrayList<>();
        ...
        query.setParameter("inverseRelationshipType",
            getInverseRelationshipType(getProject().getTerminology(), getProject().getVersion(),
                identity.getRelationshipType()));
        ...
        if (keepDuplicateIdentity != null && keepInverseIdentity == null) {
          ...
          if (inverseRelationshipIds.contains(keepDuplicateIdentity.getInverseId())) {
            keepInverseIdentity =
                identityService.getRelationshipIdentity(keepDuplicateIdentity.getInverseId());
          }
          // Otherwise, look for an inverse that points to the duplicate
            ...
            for (RelationshipIdentity relationshipIdentity : inverseRelationshipIdentities) {
              if (relationshipIdentity.getInverseId().equals(keepDuplicateIdentity.getId())) {
        ...
        // if an inverse identity is linked but not a duplicate
          ...
          if (duplicateRelationshipIds.contains(keepInverseIdentity.getInverseId())) {
            keepDuplicateIdentity =
                identityService.getRelationshipIdentity(keepInverseIdentity.getInverseId());
          }
          // Otherwise, look for a duplicate that points to the inverse
            ...
            for (RelationshipIdentity relationshipIdentity : duplicateRelationshipIdentities) {
              if (relationshipIdentity.getInverseId().equals(keepInverseIdentity.getId())) {
        ...
        if (keepDuplicateIdentity != null && keepInverseIdentity != null) {
          if (!keepDuplicateIdentity.getInverseId().equals(keepInverseIdentity.getId())) {
          ...
          if (!keepInverseIdentity.getInverseId().equals(keepDuplicateIdentity.getId())) {
          ...
          for (final RelationshipIdentity relationshipIdentity : duplicateRelationshipIdentities) {
            if (relationshipIdentity.getId().equals(keepDuplicateIdentity.getId())) {
              continue;
            } else {
              identityService.removeRelationshipIdentity(relationshipIdentity.getId());
              removedIdentities++;
            }
          }
          ...
          for (final RelationshipIdentity relationshipIdentity : inverseRelationshipIdentities) {
            if (relationshipIdentity.getId().equals(keepInverseIdentity.getId())) {
              continue;
            } else {
              identityService.removeRelationshipIdentity(relationshipIdentity.getId());
              removedIdentities++;
    ...
    logInfo("Removed " + removedIdentities + " bad inverse relationship identities.");
  ...
  private void fixComponentInfoRelationships() throws Exception {
    ...
    try {
      ...
      logInfo("[FixComponentInfoRelationships] " + componentInfoRelationships.size()
      ...
      for (final ComponentInfoRelationship componentInfoRelationship : componentInfoRelationships) {
        if (ConfigUtility.isEmpty(componentInfoRelationship.getFromTerminologyId())
            && componentInfoRelationship.getFromName().equals("NCI Thesaurus")) {
          componentInfoRelationship.setFromTerminologyId("31926003");
          updateRelationship(componentInfoRelationship);
          updatedRelationships++;
        } else if (ConfigUtility.isEmpty(componentInfoRelationship.getToTerminologyId())
  ...
  private void setComponentInfoRelationshipsToPublishable() throws Exception {
    ...
    try {
      ...
      logInfo("[SetComponentInfoRelationshipsToPublishable] " + componentInfoRelationships.size()
  ...
  private void setStampedWorklistsToReadyForPublication() throws Exception {
    ...
    try {
      Query query = getEntityManager()
          .createNativeQuery("select id from worklists where workflowStatus='REVIEW_DONE'");
      ...
      logInfo("[SetStampedWorklistsToReadyForPublication] " + stampedWorklists.size()
    ...
    logInfo("Updated " + updatedRelationships + " Stamped worklists' workflow status.");
    logInfo("Finished " + getName());
  }
  ...
  private void addDispositionAtoms() throws Exception {
    ...
    try {
      ...
      Query query = getEntityManager()
          .createNativeQuery("select c.id from concepts c, concepts_atoms ca, atoms a "
    ...
    logInfo(
        "Skipped " + skippedConceptCount + " concepts that had more than one disposition atom.");
    logInfo("Finished " + getName());
  }
  ...
  private void fixRelGroups() throws Exception {
    ...
    try {
      ...
      // REAL QUERY
      Query query = getEntityManager()
          .createNativeQuery("select id from concept_relationships where relGroup is null");

      // TEST QUERY
      // Query query = getEntityManager().createNativeQuery(
      // "select id from concept_relationships where relGroup is null limit 5");

      logInfo("[FixRelGroups] Identifying " + "relationships with rel groups set to NULL");

      List<Object> list = query.getResultList();
      for (final Object entry : list) {
        final Long id = Long.valueOf(entry.toString());
        relationships
            .add((ConceptRelationshipJpa) getRelationship(id, ConceptRelationshipJpa.class));
      }

      setSteps(relationships.size());

      logInfo("[FixRelGroups] " + relationships.size() + " Relationships identified");
    ...
    logInfo("Updated " + updatedRelationships + " relationship relGroups to blank.");
    logInfo("Finished " + getName());
  }
  ...
  private void fixSourceLevelRels() throws Exception {
    ...
    try {
      ...
      logInfo("[FixSourceLevelRels] Identifying " + "source-level relationships with status=N");
      ...
      logInfo("[FixSourceLevelRels] " + relationships.size() + " Relationships identified");
    ...
    logInfo("Updated " + updatedRelationships + " source-level relationships to status=R.");
    logInfo("Finished " + getName());
  }
    ...
    try {
      ...
      for (final Object entry : list) {
        ...
        additionalRelationshipsTypes
            .add((AdditionalRelationshipTypeJpa) getAdditionalRelationshipType(abbreviation,
                "NCIMTH", "latest"));
      }

      setSteps(additionalRelationshipsTypes.size());

      logInfo("[FixAdditionalRelTypeInverses] " + additionalRelationshipsTypes.size()
      ...
      for (final AdditionalRelationshipType additionalRelationshipType : additionalRelationshipsTypes) {
        ...
        // correct inverse
          ...
          AdditionalRelationshipType inverseRelType =
              getAdditionalRelationshipType("may_treat_MEDRT", "NCIMTH", "latest");
  ...
  private void fixSnomedFamily() throws Exception {
    ...
    // 8/23/2019 Added fix for CCS_10 source family being CCS, not CCS_10.
    // 8/2/2020 Added fix for CCSR_10 source family being CCS, not CCS_10
  ...
  private void fixTerminologyNames() throws Exception {
    ...
    try {
      ...
      for (final Terminology terminology : terminolgyList.getObjects()) {
        ...
        if (terminology.getRootTerminology().getFamily().equals("NCI")
            || terminology.getRootTerminology().getFamily().equals("SNOMEDCT_US")
        ...
        if (terminology.getPreferredName().endsWith(versionSuffix)) {
          ...
          terminology.setPreferredName(terminology.getPreferredName() + versionSuffix);
    ...
    logInfo("Updated " + updatedTerminologies + " terminology names to contain version.");
    logInfo("Finished " + getName());
  }
  ...
  private void fixTerminologies() throws Exception {
    ...
    try {
      ...
      for (final Terminology terminology : terminolgyList.getObjects()) {
        ...
        if (segments.length >= 2
            && segments[segments.length - 1].equals(segments[segments.length - 2])) {
    ...
    logInfo("Updated " + updatedTerminologies + " terminology names to remove duplicate versions.");
    logInfo("Finished " + getName());
  }
  ...
  private void fixRHTAtoms() throws Exception {
    ...
    try {
      ...
      // Get the three affected additional relationship types
      Query query = getEntityManager()
          .createNativeQuery("select id from atoms where termType='RHT' and terminology='NCIMTH'");
  ...
  private void fixMDRDescriptors() throws Exception {
    ...
    try {
      ...
      logInfo(
          "[FixMDRDesciptors] " + list.size() + " descriptors created by insertions identified");
    ...
    logInfo("Removed " + removedDescriptors + " MDR descriptors created by insertions.");
    logInfo("Updated " + updatedDescriptors + " loaded MDR descriptors tp publishable=true.");
    logInfo("Finished " + getName());
  ...
  private void removeOldWorklistsChecklists() throws Exception {
    ...
    // Get worklists
    Query query =
        getEntityManager().createQuery("select a.id from " + "WorklistJpa a where epoch = '17b'");
    ...
    query = getEntityManager()
        .createQuery("select a.id from " + "ChecklistJpa a where timestamp < '2018-08-23'");
    ...
    logInfo("[RemoveOldWorklistsChecklists] " + removals + " lists successfully removed.");
  ...
  private void removeSNOMEDSubsets() throws Exception {
    ...
    try {
      ...
      for (final Object entry : list) {
        final Long id = Long.valueOf(entry.toString());
        conceptSubsets.add((ConceptSubsetJpa) getSubset(id, ConceptSubsetJpa.class));
      }

      setSteps(conceptSubsets.size());

      logInfo("[RemoveSNOMEDSubsets] " + conceptSubsets.size() + " Concept Subsets identified");

      for (final ConceptSubsetJpa subset : conceptSubsets) {
        logInfo("[RemoveSNOMEDSubsets] " + subset.getMembers().size()
            + " Before removal concept Subset Members  identified on: " + subset.getTerminologyId()
            + " " + subset.getId());
        ...
        logInfo("[RemoveSNOMEDSubsets] " + subset.getMembers().size()
            + " After removal concept Subset Members  identified on: " + subset.getTerminologyId()
            + " " + subset.getId());
        updateProgress();
      }
    } catch (Exception e) {
  ...
  private void removeSNOMEDAtomSubsets() throws Exception {
    ...
    try {
      ...
      for (final Object entry : list) {
        final Long id = Long.valueOf(entry.toString());
        AtomSubsetJpa atomSubset = (AtomSubsetJpa) getSubset(id, AtomSubsetJpa.class);
      ...
      logInfo("[RemoveSNOMEDSubsets] " + atomSubsets.size() + " Atom Subsets identified");
      logInfo(
          "[RemoveSNOMEDSubsets] " + totalSubsetMembersSize + " Atom Subset Members identified");
      ...
      for (final AtomSubsetJpa subset : atomSubsets) {
        logInfo("[RemoveSNOMEDSubsets] " + subset.getMembers().size()
            + " Before removal atom Subset Members  identified on: " + subset.getTerminologyId()
            + " " + subset.getId());
        for (final AtomSubsetMember member : subset.getMembers()) {
          for (final Attribute att : member.getAttributes()) {
            removeAttribute(att.getId());
        ...
        logInfo("[RemoveSNOMEDSubsets] " + subset.getMembers().size()
            + " After removal atom Subset Members  identified on: " + subset.getTerminologyId()
            + " " + subset.getId());
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private void removeConceptsWithoutAtoms() throws Exception {
    ...
    try {
      ...
      for (final Object entry : conceptsWithoutAtoms) {
        ...
        if (concept == null) {
          logWarn(
              "[AdHoc Algorithm] Concept designated to be marked unpublished that is null:" + id);
          continue;
        }
    ...
    logInfo("Marked unpublishable " + markedConcepts + " concepts without atoms.");
    logInfo("Finished " + getName());
  ...
  private void fixDuplicatePDQMappingAttributes() throws Exception {
    ...
    try {
      ...
      Query query = getEntityManager().createQuery("select a.id from " + "CodeJpa a "
    ...
    logInfo("Marked unpublishable " + markedAttributes + " duplicate PDQ mapping attributes.");
    logInfo("Finished " + getName());
  ...
  private void fixDuplicateCUIs() throws Exception {
    ...
    try {
      ...
      logInfo("[FixDuplicateCUIs] " + duplicateCUIs.size() + " CUI duplicates identified");
      ...
      for (final String cui : duplicateCUIs) {
        ...
        if (list.size() != 2) {
          throw new Exception("ERROR: " + list.size() + " concepts all have the same CUI: " + cui);
        }
  ...
  private void fixNullRUIs() throws Exception {
    ...
    try {
      ...
      for (final RelationshipType rel : getRelationshipTypes(getProject().getTerminology(),
          getProject().getVersion()).getObjects()) {
        relTypeMap.put(rel.getAbbreviation(), rel.getInverse().getAbbreviation());
      }
      for (final AdditionalRelationshipType rel : getAdditionalRelationshipTypes(
          getProject().getTerminology(), getProject().getVersion()).getObjects()) {
        relTypeMap.put(rel.getAbbreviation(), rel.getInverse().getAbbreviation());
      }
      ...
      logInfo(
          "[FixNullRUIs] " + relsToFix.size() + " Concept relationships identified with null RUIs");

      setSteps(relsToFix.size());
      for (Long relId : relsToFix) {
        ConceptRelationship relationship =
            (ConceptRelationship) getRelationship(relId, ConceptRelationshipJpa.class);
  ...
  private void removeOldMTHRelationships() throws Exception {
    ...
    // version.
	// 07/31/2023 - still having same issue in MTH_2023AA.  Updated to new
	// version.
    ...
    Query query = getEntityManager().createNativeQuery(
        "select cr.id from " + " concept_relationships cr, concepts c1, concepts c2 "
            ...
            + " and cr.terminology = 'MTH' " + " and cr.terminology != '2024AB' "
    ...
    logInfo("[RemoveOldMTHRelationships] Loading "
        + "ConceptRelationship ids for old relationships that now have duplicates caused by the MTH 2023AB insertion");

    List<Object> list = query.getResultList();
    setSteps(list.size());
    logInfo("[RemoveOldMTHRelationships] " + list.size() + " ConceptRelationship ids loaded");
  ...
  private void fixComponentHistoryVersion() throws Exception {
    ...
    logInfo("[FixComponentHistoryVersion] Assigning " + "associatedRelease to version field");
    ...
    try {
      Query query = getEntityManager()
          .createNativeQuery("select id from component_histories where version='latest'");

      logInfo(
          "[FixComponentHistoryVersion] Identifying " + "ComponentHistories with version 'latest'");

      List<Object> list = query.getResultList();
      for (final Object entry : list) {
        final Long id = Long.valueOf(entry.toString());
        componentHistories.add((ComponentHistoryJpa) getComponent(id, ComponentHistoryJpa.class));
      }
  ...
  private void fixAdditionalRelTypeInverses2() throws Exception {
    ...
    try {
      ...
      logInfo("[FixAdditionalRelTypeInverses2] " + additionalRelationshipsTypes.size()
          + " additional relationship types identified");
      ...
      for (final AdditionalRelationshipType additionalRelationshipType : additionalRelationshipsTypes) {
        ...
        // correct inverse
          ...
          AdditionalRelationshipType inverseRelType =
              getAdditionalRelationshipType("develops_into", "NCIMTH", "latest");
          additionalRelationshipType.setInverse(inverseRelType);
          updateAdditionalRelationshipType(additionalRelationshipType);
          updatedAdditionalRelationshipTypes++;
        }
        ...
        // Confirm that Parent_Is_NICHD is inverse of Has_NICHD_Parent
          ...
          AdditionalRelationshipType inverseRelType =
              getAdditionalRelationshipType("Parent_Is_NICHD", "NCIMTH", "latest");
          additionalRelationshipType.setInverse(inverseRelType);
          updateAdditionalRelationshipType(additionalRelationshipType);
          updatedAdditionalRelationshipTypes++;
        }
  ...
  private void removeDemotions() throws Exception {
    ...
    try {
      ...
      Query query = getEntityManager().createNativeQuery(
          "select " + "ar.id relId, " + "cr.to_id c1Id, " + "cr.from_id c2Id " + "from "
              + "atom_relationships ar, " + "concept_relationships cr, " + "concepts_atoms ca1, "
              + "concepts_atoms ca2 " + "where " + "ar.terminology = 'NCIMTH' "
              + "and cr.terminology = 'NCIMTH' " + "and ar.from_id = ca1.atoms_id "
              + "and ar.to_id = ca2.atoms_id " + "and cr.from_id = ca1.concepts_id "
              + "and cr.to_id = ca2.concepts_id " + "and ar.workflowStatus = 'DEMOTION' "
      ...
      for (final Object[] result : ids) {
        ;
        final Relationship<?, ?> rel =
            (AtomRelationship) getRelationship(Long.valueOf(result[0].toString()),
                AtomRelationshipJpa.class);
  ...
  private void reviseSemanticTypes() throws Exception {
    ...
    try {
      ...
      Query query = getEntityManager().createNativeQuery(" select distinct concepts.id "
  ...
  private void fixAtomLastReleaseCui() throws Exception {
    ...
    try {
      ...
      logInfo("[FixAtomLastReleaseCUI] Loading the AUI/CUI map for " + getProcess().getVersion()
          + " MRCONSO");
      ...
      Query query =
          getEntityManager().createNativeQuery(" select id from atoms where publishable=false");
      ...
      for (final Object entry : list) {
        ...
        String atomAUI = atom.getAlternateTerminologyIds().get(getProject().getTerminology());
        ...
        if (!auiCuiMap.containsKey(atomAUI) && atom.getTerminology().equals("MTH")
            && atom.getTermType().equals("PN") && !ConfigUtility
                .isEmpty(atom.getConceptTerminologyIds().get(getProject().getTerminology()))) {
        ...
        else if (!auiCuiMap.containsKey(atomAUI) && atom.getTerminology().equals("PDQ")
        ...
        // AUICUI map, update the atom
          ...
          final String atomlastReleaseCUI =
  ...
  private void fixAtomSuppressibleAndObsolete() throws Exception {
    ...
    try {
      ...
      Query query = getEntityManager()
          .createNativeQuery("select id from atoms where termType in ('AB','IS','OP')");
    ...
    try {
      ...
      for (final Long treePositionId : treePositionsIds) {
        ...
        final AtomTreePositionJpa atomTreePosition =
            (AtomTreePositionJpa) getTreePosition(treePositionId, AtomTreePositionJpa.class);
  ...
  private void removeOldTermgroups() throws Exception {
    ...
    logInfo("[RemoveOldTermgroups] " + termgroupsToRemove.size() + " Termgroups to be removed");

    updateProgress();

    logInfo("[RemoveOldTermgroups] " + removals + " old termgroups successfully removed.");
  ...
  public void setProperties(Properties p) throws Exception {
    ...
    if (p.getProperty("integerParameter") != null) {
        integerParameter =
            Integer.parseInt(String.valueOf(p.getProperty("integerParameter")));
    }
    if (p.getProperty("stringParameter") != null) {
        stringParameter = String.valueOf(p.getProperty("stringParameter"));
      }
  ...
  private void convertToChecklist(String name, long projectId, List<Long[]> results,
    ...
    for (final Checklist checklist : checklists.getObjects()) {
      if (checklist.getName().equals(name) && checklist.getProject().equals(project)) {
        throw new LocalException("A checklist for project " + project.getName() + " with name "
  ...
  private void fixOverlappingBRORels() throws Exception {
    ...
    logInfo("[FixOverlappingBRORelationships] " + results.size() + " Concept id pairs loaded");

    for (final Object[] entry : results) {

      final Concept fromConcept = getConcept(Long.parseLong(entry[0].toString()));
      ...
      final List<ConceptRelationship> overlappingRelationships = new ArrayList<>();
      ...
      if (overlappingRelationships.size() < 2) {
        throw new Exception("Unexpectedly unable to find multiple relationships between concepts "
      ...
      if (originalRelationship.getRelationshipType().startsWith("B")) {
        ...
        updateConcept(fromConcept);
        removeRelationship(originalRelationship.getId(), originalRelationship.getClass());
      }
  ...
  private void combineAtomsByUMLSCui() throws Exception {
    // 12/21/2020 Consolidate all new atoms by UMLS-CUI into a single concept,
    // IF that
    ...
    try {
      ...
      logInfo("  Consolidate new atoms by UMLS-CUI where safe");
      ...
      // Now go through the new atoms and consolidate where appropriate

      int consolidationStepsCompleted = 0;

...
set exists = `$aws rds describe-db-snapshots --query "DBSnapshots[?DBSnapshotIdentifier=='$DB_NAME-$SNAPSHOT_DATE-final-snapshot'].[Status][0][0]" `
echo "exists: $exists"
...
while ($started =~ 'deleting')
   echo "deleting"
   set started = `$aws rds describe-db-instances --query "DBInstances[?DBInstanceIdentifier=='$DB_NAME'].[DBInstanceStatus][0][0]" | tr -d '\n' | tr -d '"'`
end
